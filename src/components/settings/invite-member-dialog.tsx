...
export function InviteMemberDialog({
  ...
  async function handleCreate() {
    ...
    try {
      ...
      setResult({
        ...
        accountName: account?.name ?? 'our Strive CRM account',
      });
      onCreated();
    } catch (err) {
  ...
  function whatsappShareUrl(url: string): string {
    ...
    // for users in multi-team contexts where "our Strive CRM account"
    // wouldn't be enough to disambiguate.
    const accountName = result?.accountName ?? 'our Strive CRM account';
    const message = `Join ${accountName} on Strive CRM using this link (valid for ${result?.expiresInDays} days): ${url}`;
